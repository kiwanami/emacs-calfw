...
(defun cfw:event-format-field-date (date)
  ...
  `((?Y . ,(cfw:event-format-field-number (cl-caddr date) 4))
...
(defconst cfw:calendar-buffer-name "*cfw-calendar*"
...
keymap `cfw:calendar-mode-map'."
  (let ((buffer (or buf (get-buffer-create cfw:calendar-buffer-name)))
...
(defun cfw:dest-init-inline (width height)
  ...
  (let ((buffer (get-buffer-create cfw:dest-background-buffer))
...
return an alist of rendering parameters."
  (let* ((win-width (cfw:dest-width dest))
...
(defvar-local cfw:main-buf nil)
(defvar-local cfw:before-win-num nil)
...
;; buffer
(defvar-local cfw:component nil)
